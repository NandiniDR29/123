...
import static com.exigen.ren.main.modules.policy.gb_vs.masterpolicy.tabs.PremiumSummaryTab.*;
import static com.exigen.ren.utils.components.Components.POLICY_GROUPBENEFITS;
import static com.exigen.ren.utils.groups.Groups.*;

public class TestMaintainInForceRateVerification extends BaseTest implements CustomerContext, CaseProfileContext, GroupVisionMasterPolicyContext {

    @Test(groups = {GB, GB_PRECONFIGURED, GB_VS, WITHOUT_TS, REGRESSION})
    @TestInfo(testCaseId = {"REN-35745"}, component = POLICY_GROUPBENEFITS)
    public void testMaintainInForceRateVerification_VS() {
        LOGGER.info("General Preconditions");
        String planName = groupVisionMasterPolicy.getDefaultTestData(TestDataKey.DATA_GATHER, "SelectPlan").getValue(PlanDefinitionTabMetaData.PLAN.getLabel());
        ...
        quoteInitiateAndFillUpToTab(getDefaultVSMasterPolicyData(), PremiumSummaryTab.class, true);
        ...
        proposeAcceptContractIssueVSMasterPolicyWithDefaultTestData();

        LOGGER.info("Steps#3.1, 3.2 verification");
        EndorseAction.startEndorsementForPolicy(GroupBenefitsMasterPolicyType.GB_VS, groupVisionMasterPolicy.getDefaultTestData(TestDataKey.ENDORSEMENT, TestDataKey.DEFAULT_TEST_DATA_KEY));
        premiumSummaryTab.navigateToTab();
        ...
        assertSoftly(softly -> {
            ...
            premiumSummaryTab.getAssetList().getAsset(PremiumSummaryTabMetaData.EXPERIENCE_RATING).getAsset(PremiumSummaryTabMetaData.ExperienceRatingMetaData.EXPERIENCE_CLAIM_AMOUNT).setValue("10");
            premiumSummaryTab.rate();
