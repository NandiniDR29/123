...
public class TestPolicyCancelNoticeAddDelete extends BaseTest implements CustomerContext, CaseProfileContext, GroupVisionMasterPolicyContext {
    ...
    public void testPolicyCancelNoticeAddDelete() {
        ...
        groupVisionMasterPolicy.cancelNotice().perform(groupVisionMasterPolicy.getDefaultTestData(TestDataKey.CANCEL_NOTICE, TestDataKey.DEFAULT_TEST_DATA_KEY));
        assertThat(PolicySummaryPage.labelCancelNotice).isPresent();

        LOGGER.info("TEST: Delete Cancel Notice for Policy #" + policyNumber);
        groupVisionMasterPolicy.removeCancelNotice().perform(new SimpleDataProvider());
