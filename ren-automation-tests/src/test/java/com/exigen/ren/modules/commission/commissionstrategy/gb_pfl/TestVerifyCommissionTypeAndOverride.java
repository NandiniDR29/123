package com.exigen.ren.modules.commission.commissionstrategy.gb_pfl;
...
import static com.exigen.ren.main.enums.CoveragesConstants.GroupPFLCoverages.PFL;
...
public class TestVerifyCommissionTypeAndOverride extends CommissionStrategyBaseTest implements CustomerContext, CaseProfileContext, PaidFamilyLeaveMasterPolicyContext {
    ...
    private void commonSteps(String testDataName, String columnName, String commissionType, String columnNameForOverrideSection) {
        ...
        paidFamilyLeaveMasterPolicy.getDefaultWorkspace().fillFromTo(getDefaultPFLMasterPolicyData()
                        ...
                        .adjust(TestData.makeKeyPath(policyInformationTab.getMetaKey(), NEXT_RENEWAL_QUOTE_START_DATE.getLabel()),
                                TimeSetterUtil.getInstance().getCurrentTime().format(DateTimeUtils.MM_DD_YYYY))
        ...
        Button overrideCommissionButton = PremiumSummaryTab.getCommissionOverrideButtonForAgencyWithCoverage(AGENCY_NAME, PFL);
        assertSoftly(softly -> {
            softly.assertThat(overrideCommissionButton).isPresent();
            softly.assertThat(PremiumSummaryTab.getCommissionTypeByAgency(AGENCY_NAME, PFL)).isEqualTo(commissionType);

        });
        overrideCommissionButton.click();
        DialogOverrideCommissionPremiumSummary.commissionOverrideOption.setValueContains("Name");
        commissionRateInfoTable.getRowContains(DialogOverrideCommissionPremiumSummary.CommissionRateInfo.COVERAGE_NAME.getName(), PFL).getCell(1).click();
