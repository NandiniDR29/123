...
import static com.exigen.ren.main.enums.PolicyConstants.PlanLTD.LTD_VOL;
...
import static com.exigen.ren.main.modules.policy.gb_di_ltd.masterpolicy.tabs.PremiumSummaryTab.buttonViewRateHistory;
...
public class TestSalesforceProposedQuoteMappingVerificationForLTD extends SalesforceBaseTest implements CustomerContext, CaseProfileContext, LongTermDisabilityMasterPolicyContext {

    @Test(groups = {WITHOUT_TS, REGRESSION, INTEGRATION})
    @TestInfo(testCaseId = {"REN-43733"}, component = INTEGRATION)
    public void testSalesforceProposedQuoteMappingVerificationFor_LTD() {
        ...
        tableRateHistory.getRowContains(TableConstants.RateHistoryTable.PLAN_NAME.getName(), LTD_CON).getCell(1).controls.links.getFirst().click();
        ...
        tableRateHistory.getRowContains(TableConstants.RateHistoryTable.PLAN_NAME.getName(), LTD_VOL).getCell(1).controls.links.getFirst().click();
        tableClassificationGroups.getRow(1).getCell(1).controls.links.getFirst().click();
        ...
        String quoteIdCoverage1 = String.format("%s_%s_%s", quoteNumber, LTD_CON, SF_LTD);
        String quoteIdCoverage2 = String.format("%s_%s_%s", quoteNumber, LTD_VOL, SF_LTD_VOLUNTARY);
