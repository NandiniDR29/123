...
public class TestWarnsIfClaimsAreAffectedWhenCreateEndorsement extends ClaimGroupBenefitsTLBaseTest {
    ...
    public void testCheckWarningsIfClaimsAffectedDuringEndorse() {
        ...
        claim.create(claim.getDefaultTestData(DATA_GATHER_CERTIFICATE, TestDataKey.DEFAULT_TEST_DATA_KEY));

        LOGGER.info("REN-30759 TC#2 Scenario for Master Policy (endorsement)");
        endorsementCommonSteps(policies.get(0), "Acquisition or Merger", termLifeInsuranceMasterPolicy);

        LOGGER.info("REN-30759 TC#2 Scenario for Certificate Policy (endorsement)");
        endorsementCommonSteps(policies.get(1), "Marriage", termLifeInsuranceCertificatePolicy);
    }
    ...
    public void testCheckWarningsIfClaimsAffectedDuringCancellationMasterCertPolicy() {
        ...
        claim.calculateSingleBenefitAmount().perform(claim.getDefaultTestData("CalculateASingleBenefitAmount", TestDataKey.DEFAULT_TEST_DATA_KEY)
                .adjust(makeKeyPath(CoveragesActionTab.class.getSimpleName(), CoveragesActionTabMetaData.INDEMNITY_RESERVE.getLabel()), "1000"), 1);
        claim.postRecovery().perform(claim.getDefaultTestData("ClaimPayment", "TestData_PostRecovery"));

        LOGGER.info("REN-30770 TC#2 Cancellation action Certificate Policy");
        cancelCommonSteps(policies.get(1), "Term", termLifeInsuranceCertificatePolicy);

        LOGGER.info("REN-30769 TC#2 Select Cancellation action Master Policy");
        cancelCommonSteps(policies.get(0), "Non Payment of Premium", termLifeInsuranceMasterPolicy);
    }
    ...
    private void cancelCommonSteps(String policyNumber, String endorsementReason, IPolicy policy) {
        ...
        policy.cancel().getWorkspace().getTab(CancellationActionTab.class).getAssetList()
                .getAsset(CancellationActionTabMetaData.CANCELLATION_REASON).setValue(endorsementReason);
    ...
    private void endorsementCommonSteps(String policyNumber, String endorsementReason, IPolicy policy) {
        ...
        policy.endorse().getWorkspace().getTab(StartEndorsementActionTab.class).getAssetList()
                .getAsset(StartEndorsementActionTabMetaData.ENDORSEMENT_REASON).setValue(endorsementReason);
