...
import static com.exigen.ren.main.modules.policy.gb_di_std.masterpolicy.metadata.PlanDefinitionTabMetaData.ELIGIBILITY;
import static com.exigen.ren.main.modules.policy.gb_di_std.masterpolicy.metadata.PlanDefinitionTabMetaData.EligibilityMetaData.WAITING_PERIOD_DEFINITION;
...
public class TestVerifyRulesOfCalculatedEffectiveDateInitialLoad extends RestBaseTest implements CaseProfileContext, ShortTermDisabilityMasterPolicyContext {

    @Test(groups = {GB, GB_PRECONFIGURED, GB_DI_STD, WITH_TS, REGRESSION})
    @TestInfo(testCaseId = "REN-36610", component = POLICY_GROUPBENEFITS)
    public void testVerifyRulesOfCalculatedEffectiveDateInitialLoad() {
        ...
        caseProfile.create(CaseProfileContext.getDefaultCaseProfileTestData(shortTermDisabilityMasterPolicy.getType())
                ...
                .adjust(makeKeyPath(fileIntakeManagementTab.getMetaKey() + "[0]", FileIntakeManagementTabMetaData.EFFECTIVE_DATE.getLabel()), effectiveDate.format(MM_DD_YYYY)));

        shortTermDisabilityMasterPolicy.createPolicy(getDefaultSTDMasterPolicyData().adjust(tdSpecific().getTestData("TestData_Policy").resolveLinks())
                .adjust(makeKeyPath(planDefinitionTab.getMetaKey() + "[1]", ELIGIBILITY.getLabel(), WAITING_PERIOD_DEFINITION.getLabel()),
