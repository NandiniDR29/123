...
public class TestCustomerCampaignArchive extends CemBaseTest {
    ...
    @TestInfo(testCaseId = "IPBQA-24809", component = CRM_CUSTOMER)
    public void testCustomerCampaignArchive() {
        ...
        LOGGER.info("TEST: Archive Campaign # " + campaignId);
        campaign.archiveCampaign(1);

        campaign.search(tdCampaign.getTestData("SearchData", "TestData").adjust(TestData.makeKeyPath(
                CemPage.SearchCem.class.getSimpleName(), CemPage.SearchCem.CAMPAIGN_ID.getLabel()), campaignId));

        assertThat(CampaignPage.tableCampaigns.getRow(1)).isAbsent();
    }
}
