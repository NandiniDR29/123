...
public class TestPolicyCancelNoticeAddDelete extends BaseTest implements CustomerContext, CaseProfileContext, EmployeeAssistanceProgramMasterPolicyContext {
    ...
    public void testPolicyCancelNoticeAddDelete() {
        ...
        employeeAssistanceProgramMasterPolicy.cancelNotice().perform(employeeAssistanceProgramMasterPolicy.getDefaultTestData(TestDataKey.CANCEL_NOTICE, TestDataKey.DEFAULT_TEST_DATA_KEY));
        assertThat(PolicySummaryPage.labelCancelNotice).isPresent();

        LOGGER.info("TEST: Delete Cancel Notice for Policy #" + policyNumber);
        employeeAssistanceProgramMasterPolicy.removeCancelNotice().perform(new SimpleDataProvider());
