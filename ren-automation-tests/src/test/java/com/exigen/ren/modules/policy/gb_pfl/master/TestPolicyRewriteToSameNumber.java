...
public class TestPolicyRewriteToSameNumber extends BaseTest implements CustomerContext, CaseProfileContext, PaidFamilyLeaveMasterPolicyContext {
    ...
    public void testPolicyRewriteToSameNumber() {
        ...
        paidFamilyLeaveMasterPolicy.createRewrite(paidFamilyLeaveMasterPolicy.getDefaultTestData(TestDataKey.REWRITE, TestDataKey.DEFAULT_TEST_DATA_KEY)
                .adjust(paidFamilyLeaveMasterPolicy.getDefaultTestData(TestDataKey.REWRITE, TestDataKey.DATA_GATHER).resolveLinks())
                .adjust(paidFamilyLeaveMasterPolicy.getDefaultTestData(TestDataKey.ACCEPT_CONTRACT, TestDataKey.DEFAULT_TEST_DATA_KEY).resolveLinks())
                .adjust(paidFamilyLeaveMasterPolicy.getDefaultTestData(TestDataKey.ISSUE, TestDataKey.EXISTENT_BILLING_ACCOUNT).resolveLinks()));
        ...
        String rewriteEffectiveDate = paidFamilyLeaveMasterPolicy.getDefaultTestData(TestDataKey.REWRITE, TestDataKey.DEFAULT_TEST_DATA_KEY).getValue(RewriteActionTab.class.getSimpleName(),
                RewriteActionTabMetaData.EFFECTIVE_DATE.getLabel());

        assertSoftly(softly -> {
            assertThat(NotesAndAlertsSummaryPage.activitiesAndUserNotes.getRow(1)).hasCellWithValue("Description", String.format(BamConstants.POLICY_EFFECTIVE_FROM_QUOTE, policyNumber, rewriteEffectiveDate));
