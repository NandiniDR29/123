...
public class TestRestCustomerGetOpportunity extends RestBaseTest {
    private TestData opportunityTab = tdCustomerNonIndividual.getTestData("Opportunity").getTestData("Opportunity_Rest", OpportunityActionTab.class.getSimpleName());
    ...
    public void testRestCustomerGetOpportunity() {
        ...
        customerNonIndividual.addOpportunity().perform(tdCustomerNonIndividual.getTestData("Opportunity", "Opportunity_Rest"));
    ...
    private void checkResponse(OpportunityModel opportunityModel, int responseCode, Boolean byId) {
        assertSoftly(softly -> {
            ...
            softly.assertThat(opportunityModel.getDescription()).isEqualTo(opportunityTab.getValue(DESCRIPTION.getLabel()));
            softly.assertThat(opportunityModel.getOpportunityChannelCd()).isEqualTo(opportunityTab.getValue(CHANNEL.getLabel()).toUpperCase());
            softly.assertThat(opportunityModel.getOpportunityLikelihoodCd()).isEqualTo(opportunityTab.getValue(LIKELIHOOD.getLabel()).toUpperCase());
            softly.assertThat(opportunityModel.getPotential().getAmount()).isEqualTo(Double.valueOf(opportunityTab.getValue(POTENTIAL.getLabel())));
            ...
            if (byId) {
                softly.assertThat(opportunityModel.getReferral().getDisplayValue())
                        .contains(opportunityTab.getTestData(REFERRED_BY.getLabel()).getValue(SearchReferralMetaData.FIRST_NAME.getLabel()));
                softly.assertThat(opportunityModel.getOwner().getQueueName())
                        .contains(opportunityTab.getTestData(OPPORTUNITY_OWNER.getLabel()).getValue(SearchOwnerMetaData.WORK_QUEUE.getLabel()));
            }
        });
    }
}
