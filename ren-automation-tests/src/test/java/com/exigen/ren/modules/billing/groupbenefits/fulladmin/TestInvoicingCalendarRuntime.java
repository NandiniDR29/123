...
public class TestInvoicingCalendarRuntime extends GroupBenefitsBillingBaseTest implements BillingAccountContext, GroupVisionMasterPolicyContext {
    ...
    public void testInvoicingCalendarRuntime() {
        ...
        createDefaultCaseProfile(groupAccidentMasterPolicy.getType(), groupVisionMasterPolicy.getType());

        billingAccount.navigateToBillingAccount();
        billingAccount.addManageInvoicingCalendars().perform(billingAccount.getDefaultTestData("CreateCalendars", "TestDataFullAdmin"));
        ...
        TestData td = groupVisionMasterPolicy.getDefaultTestData(DATA_GATHER, DEFAULT_TEST_DATA_KEY)
                .adjust(TestData.makeKeyPath(GroupVisionMasterPolicyContext.policyInformationTab.getClass().getSimpleName(), SITUS_STATE.getLabel()), "VT")
                ...
                .adjust(groupVisionMasterPolicy.getDefaultTestData(TestDataKey.ISSUE, "TestDataWithExistingBA"));
        groupVisionMasterPolicy.createPolicy(td);
        ...
        LOGGER.info("---=={Step 7}==---");
        List<TestData> tdList = billingAccount.getDefaultTestData("CreateCalendars", "TestDataFullAdmin").getTestDataList(
        ...
        geography.getAsset(OPEN_POPUP).click();
        geography.getAsset(STATES).unsetAllValues();
        geography.getAsset(STATES).setValue(ImmutableList.of("GA", "VT"));
        geography.getAsset(SUBMIT_POPUP).click();
        Tab.buttonSave.click();
        Tab.buttonBack.click();
        assertThat(updateBillingAccountActionTab.getAssetList().getAsset(INVOICING_CALENDAR)).containsAllOptions(ImmutableList.of("", "Default Invoicing Calendar1", calendarNameIC1, calendarNameIC2));
        ...
        LOGGER.info("---=={Step 14}==---");
        billingAccount.addManageInvoicingCalendars().getWorkspace().getTab(ManageInvoicingCalendarsActionTab.class).getAssetList().getAsset(PRODUCTS).setValue(ImmutableList.of("Group Accident"));
        Tab.buttonSave.click();
        Tab.buttonBack.click();
        ...
        ManageInvoicingCalendarsActionTab.tableInvoicingCalendars.getRow(ImmutableMap.of(CALENDAR_NAME.getName(), calendarNameIC2))
                .getCell(ManageInvoicingCalendarsActionTab.InvoicingCalendars.ACTIONS.getName()).controls.links.get(ActionConstants.VIEW_EDIT).click();
        billingAccount.addManageInvoicingCalendars().getWorkspace().getTab(ManageInvoicingCalendarsActionTab.class).getAssetList().getAsset(PRODUCTS).setValue(ImmutableList.of("Group Accident", "Group Vision"));
        Tab.buttonSave.click();
        Tab.buttonBack.click();
        assertThat(updateBillingAccountActionTab.getAssetList().getAsset(INVOICING_CALENDAR)).containsAllOptions(ImmutableList.of("", "Default Invoicing Calendar1", calendarNameIC1, calendarNameIC2));
