...
public class TestPolicyCancelNoticeAddDelete extends BaseTest implements CustomerContext, CaseProfileContext, StatutoryDisabilityInsuranceMasterPolicyContext {
    ...
    public void testPolicyCancelNoticeAddDelete() {
        ...
        statutoryDisabilityInsuranceMasterPolicy.cancelNotice().perform(statutoryDisabilityInsuranceMasterPolicy.getDefaultTestData(TestDataKey.CANCEL_NOTICE, TestDataKey.DEFAULT_TEST_DATA_KEY));
        assertThat(PolicySummaryPage.labelCancelNotice).isPresent();

        LOGGER.info("TEST: Delete Cancel Notice for Policy #" + policyNumber);
        statutoryDisabilityInsuranceMasterPolicy.removeCancelNotice().perform(new SimpleDataProvider());
