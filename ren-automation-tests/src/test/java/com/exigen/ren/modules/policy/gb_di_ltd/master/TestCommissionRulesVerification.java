...
import static com.exigen.ren.main.modules.policy.gb_di_ltd.masterpolicy.metadata.PolicyInformationTabMetaData.*;
import static com.exigen.ren.utils.AdminActionsHelper.createUserAndRelogin;
import static com.exigen.ren.utils.components.Components.POLICY_GROUPBENEFITS;
import static com.exigen.ren.utils.groups.Groups.*;

public class TestCommissionRulesVerification extends BaseTest implements CustomerContext, CaseProfileContext, LongTermDisabilityMasterPolicyContext {
    ...
    @TestInfo(testCaseId = {"REN-25539"}, component = POLICY_GROUPBENEFITS)
    public void testCommissionRulesVerification_LTD() {
        ...
        policyInformationTab.getAssetList().getAsset(ASSIGNED_AGENCIES).getAsset(PolicyInformationTabMetaData.AssignedAgenciesMetaData.AGENCY_PRODUCER).setValue(agencyName2);
        ...
        longTermDisabilityMasterPolicy.propose().getWorkspace().getTab(ProposalActionTab.class)
        ...
        longTermDisabilityMasterPolicy.copyQuote().perform(longTermDisabilityMasterPolicy.getDefaultTestData("CopyFromQuote", TestDataKey.DEFAULT_TEST_DATA_KEY));

        LOGGER.info("Step#15 verification");
        longTermDisabilityMasterPolicy.dataGather().start();
        ImmutableList agencies = ImmutableList.of(agencyName3, agencyName4, agencyName5, agencyName6);
        agencies.forEach(x -> {
            policyInformationTab.getAssetList().getAsset(AssignedAgenciesMetaData.ADD_AGENCY).click();
        ...
        LongTermDisabilityMasterPolicyContext.classificationManagementMpTab.navigateToTab();
        ...
        LongTermDisabilityMasterPolicyContext.classificationManagementMpTab.submitTab();
        ...
        longTermDisabilityMasterPolicy.propose().start();
        longTermDisabilityMasterPolicy.propose().getWorkspace().fillUpTo(getDefaultLTDMasterPolicyData()
        ...
        longTermDisabilityMasterPolicy.propose().getWorkspace().getTab(ProposalActionTab.class).fillTab(getDefaultLTDMasterPolicyData()
    ...
    private void overrideCommission(String agencyName) {
        Button overrideCommissionButton = PremiumSummaryTab.getCommissionOverrideButtonForAgencyWithCoverage(agencyName, "LTD Core");
        overrideCommissionButton.click();
        DialogOverrideCommissionPremiumSummary.commissionOverrideOption.setValue("LTD-Flat");
        DialogOverrideCommissionPremiumSummary.overrideCommissionForAllCoverages("11");//value 31 was changed due to REN-43232
