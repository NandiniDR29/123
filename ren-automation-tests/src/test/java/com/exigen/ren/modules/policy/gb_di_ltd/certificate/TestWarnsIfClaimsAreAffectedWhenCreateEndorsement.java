...
public class TestWarnsIfClaimsAreAffectedWhenCreateEndorsement extends BaseTest implements CustomerContext, CaseProfileContext, LongTermDisabilityCertificatePolicyContext, LongTermDisabilityMasterPolicyContext {
    ...
    public void testCheckWarningsIfClaimsAffectedDuringEndorse() {
        ...
        disabilityClaim.create(disabilityClaim.getLTDTestData().getTestData(DATA_GATHER_CERTIFICATE, TestDataKey.DEFAULT_TEST_DATA_KEY));

        LOGGER.info("REN-30759 TC#2 Scenario for Master Policy (endorsement)");
        endorsementCommonSteps(policies.get(0), "Acquisition or Merger", longTermDisabilityMasterPolicy);

        LOGGER.info("REN-30759 TC#2 Scenario for Certificate Policy (endorsement)");
        endorsementCommonSteps(policies.get(1), "Marriage", longTermDisabilityCertificatePolicy);
    }
    ...
    public void testCheckWarningsIfClaimsAffectedDuringCancellationMasterCertPolicy() {
        ...
        disabilityClaim.calculateSingleBenefitAmount().perform(disabilityClaim.getLTDTestData().getTestData("CalculateASingleBenefitAmount", "TestData_LTD")
        ...
        cancelCommonSteps(policies.get(1), "Term", longTermDisabilityCertificatePolicy);

        LOGGER.info("REN-30769 TC#2 Select Cancellation action Master Policy");
        cancelCommonSteps(policies.get(0), "Non Payment of Premium", longTermDisabilityMasterPolicy);
    }
