...
import com.exigen.ren.main.modules.claim.common.tabs.EventInformationLossEventTab;
...
import static com.exigen.ren.main.modules.claim.gb_ac.AccidentHealthClaimContext.accHealthClaim;
import static com.exigen.ren.utils.components.Components.POLICY_GROUPBENEFITS;
import static com.exigen.ren.utils.groups.Groups.*;

public class TestWarnsIfClaimsAreAffectedWhenCreateEndorsement extends ScenarioTestArchiveTransactionForCertificatePolicy implements GroupAccidentMasterPolicyContext, GroupAccidentCertificatePolicyContext {
    ...
    public void testCheckWarningsIfClaimsAffectedDuringEndorse() {
        ...
        endorsementCommonSteps(policies.get(0), "Acquisition or Merger", groupAccidentMasterPolicy);

        LOGGER.info("REN-30759 TC#2 Scenario for Certificate Policy (endorsement)");
        endorsementCommonSteps(policies.get(1), "Marriage", groupAccidentCertificatePolicy);
    }
    ...
    public void testCheckWarningsIfClaimsAffectedDuringCancellationMasterCertPolicy() {
        ...
        ImmutableList<String> policies = preconditionSteps();
        accHealthClaim.create(accHealthClaim.getGbACTestData().getTestData(DATA_GATHER_CERTIFICATE, TestDataKey.DEFAULT_TEST_DATA_KEY)
                .adjust(makeKeyPath(EventInformationLossEventTab.class.getSimpleName(), EventInformationLossEventTabMetaData.ICD_CODE.getLabel()), "005D0ZZ"));
        accHealthClaim.claimSubmit().perform(new SimpleDataProvider());
        accHealthClaim.claimOpen().perform();
        accHealthClaim.calculateSingleBenefitAmount().perform(accHealthClaim.getGbACTestData().getTestData("CalculateASingleBenefitAmount", "TestData_AccidentalDeath").resolveLinks()
                .adjust(makeKeyPath(CoveragesActionTab.class.getSimpleName(), CoveragesActionTabMetaData.INDEMNITY_RESERVE.getLabel()), "1000"), 1);
        accHealthClaim.postRecovery().perform(accHealthClaim.getGbACTestData().getTestData("ClaimPayment","TestData_PostRecovery"));

        LOGGER.info("REN-30770 TC#2 Cancellation action Certificate Policy");
        cancelCommonSteps(policies.get(1), "Term", groupAccidentCertificatePolicy);

        LOGGER.info("REN-30769 TC#2 Select Cancellation action Master Policy");
        cancelCommonSteps(policies.get(0), "Non Payment of Premium", groupAccidentMasterPolicy);
    }
    ...
    private void cancelCommonSteps(String policyNumber, String endorsementReason, IPolicy policy){
        ...
        policy.cancel().getWorkspace().getTab(CancellationActionTab.class).getAssetList().getAsset(CancellationActionTabMetaData.CANCELLATION_REASON)
                .setValue(endorsementReason);
    ...
    private void endorsementCommonSteps(String policyNumber, String endorsementReason, IPolicy policy){
        ...
        policy.endorse().getWorkspace().getTab(StartEndorsementActionTab.class).getAssetList().getAsset(StartEndorsementActionTabMetaData.ENDORSEMENT_REASON)
                .setValue(endorsementReason);
    ...
    private ImmutableList<String> preconditionSteps(){
        ...
        TestData td = caseProfile.getDefaultTestData().getTestData("CaseProfile", fileIntakeManagementTab.getMetaKey())
                .adjust(CaseProfileDetailsTabMetaData.EFFECTIVE_DATE.getLabel(), DateTimeUtils.getCurrentDateTime().minusMonths(1).format(MM_DD_YYYY));
        caseProfile.create(CaseProfileContext.getDefaultCaseProfileTestData(groupAccidentMasterPolicy.getType())
                ...
                .adjust(fileIntakeManagementTab.getMetaKey(), td));
