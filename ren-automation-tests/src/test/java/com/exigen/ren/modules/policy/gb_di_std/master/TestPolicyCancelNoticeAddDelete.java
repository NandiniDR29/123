...
public class TestPolicyCancelNoticeAddDelete extends BaseTest implements CustomerContext, CaseProfileContext, ShortTermDisabilityMasterPolicyContext {
    ...
    public void testPolicyCancelNoticeAddDelete() {
        ...
        shortTermDisabilityMasterPolicy.cancelNotice().perform(shortTermDisabilityMasterPolicy.getDefaultTestData(TestDataKey.CANCEL_NOTICE, TestDataKey.DEFAULT_TEST_DATA_KEY));
        assertThat(PolicySummaryPage.labelCancelNotice).isPresent();

        LOGGER.info("TEST: Delete Cancel Notice for Policy #" + policyNumber);
        shortTermDisabilityMasterPolicy.removeCancelNotice().perform(new SimpleDataProvider());
