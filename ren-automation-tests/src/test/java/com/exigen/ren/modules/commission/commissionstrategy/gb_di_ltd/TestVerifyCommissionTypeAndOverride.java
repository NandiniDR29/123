...
import static com.exigen.ren.main.enums.CoveragesConstants.GroupLTDCoverages.LTD_CORE;
...
public class TestVerifyCommissionTypeAndOverride extends CommissionStrategyBaseTest implements CustomerContext, CaseProfileContext, LongTermDisabilityMasterPolicyContext {
    ...
    private void commonSteps(String testDataName, String columnName, String commissionType, String columnNameForOverrideSection) {
        ...
        longTermDisabilityMasterPolicy.getDefaultWorkspace().fillFromTo(getDefaultLTDMasterPolicyData()
        ...
        Button overrideCommissionButton = PremiumSummaryTab.getCommissionOverrideButtonForAgencyWithCoverage(AGENCY_NAME, LTD_CORE);
        assertSoftly(softly -> {
            softly.assertThat(overrideCommissionButton).isPresent();
            softly.assertThat(PremiumSummaryTab.getCommissionTypeByAgency(AGENCY_NAME, LTD_CORE)).isEqualTo(commissionType);

        });
        overrideCommissionButton.click();
        DialogOverrideCommissionPremiumSummary.commissionOverrideOption.setValueContains("Name");
        commissionRateInfoTable.getRowContains(DialogOverrideCommissionPremiumSummary.CommissionRateInfo.COVERAGE_NAME.getName(), LTD_CORE).getCell(1).click();
