...
public class TestBillByLocOneCovTwoLocsDifferBGDifferBA extends GroupBenefitsBillingBaseTest implements BillingAccountContext {
    @Test(groups = {BILLING_GB, REGRESSION, WITHOUT_TS})
    @TestInfo(testCaseId = "REN-42274", component = BILLING_GROUPBENEFITS)
    public void testBillByLocOneCovTwoLocsDifferBillGroupsDifferBA() {
        mainApp().open();
...
//        MP1 (Coverages - COV1 Payment Mode is 12, calendar Monthly).
        ...
        Cell firstBillingAccountCell = tableBenefitAccounts.getRow(2).getCell(BillingConstants.BillingBenefitAccountsTable.BILLING_ACCOUNT);
        ...
        firstBillingAccountCell.controls.links.getFirst().click();
        ...
        assertThat(tableModalPremium).hasRows(2);
        assertSoftly(softly -> {
            softly.assertThat(tableModalPremium).hasMatchingRows(1, ImmutableMap.of(ModalPremiums.STATUS.getName(), "Inactive"));
            softly.assertThat(tableModalPremium).hasMatchingRows(1, ImmutableMap.of(ModalPremiums.STATUS.getName(), "Active"));
        });
        ...
        assertThat((tableModalPremium.getRow(ImmutableMap.of(ModalPremiums.LOCATION.getName(), "LOC1", ModalPremiums.MODAL_PREMIUM_EFFECTIVE_DATE.getName(), currentDate)).getCell(ModalPremiums.COVERAGE.getName()).controls.links).getFirst()).isAbsent();
        ...
        billingAccount.generateFutureStatement().perform();
        assertThat(BillingSummaryPage.tableBillsAndStatements.getColumn(STATUS).getCell(1)).hasValue(ISSUED_ESTIMATED);
        String firstInvoiceNumberForSecondBA = BillingSummaryPage.tableBillsAndStatements.getRow(1).getCell(INVOICE).getValue();

        LOGGER.info("Step 8");
        BillingSummaryPage.expandBillsStatementsInvoiceViewByInvoice(firstInvoiceNumberForSecondBA);
        assertThat(tableCurrentPeriodForBillCovBillGroupsByInvoice).hasRows(1);
        assertSoftly(softly -> {
            softly.assertThat(tableCurrentPeriodForBillCovBillGroupsByInvoice).hasRowsThatContain(1, ImmutableMap.of(
                    TableConstants.BillableCoveragesBillingGroupsByInvoice.COVERAGE.getName(), "Enhanced Accident",
        ...
        LOGGER.info("Step 9");
        BillingSummaryPage.openBillsStatementsPeriodView();
        BillingSummaryPage.expandBillsStatementsPeriodViewByInvoice(firstInvoiceNumberForSecondBA);
        assertThat(tablePremiumByCoverageSegmentClassifier).hasRows(1);
        ...
        LOGGER.info("Step 10");
        String billingPeriod_BA2 = tableBillsAndStatementsByPeriod.getRow(ImmutableMap.of(BillingBillsAndStatementsGBByPeriod.INVOICE.getName(), firstInvoiceNumberForSecondBA))
                .getCell(BillingBillsAndStatementsGBByPeriod.BILLING_PERIOD.getName()).controls.links.getFirst().getValue();
        tableBillsAndStatementsByPeriod.getRow(1).getCell(ACTION.getName()).controls.links.get(ActionConstants.EDIT).click();
        assertSoftly(softly -> {
            softly.assertThat(tableAdjustPremium).hasRows(1);
            softly.assertThat(new StaticElement(By.xpath("//td[@class='section_header' and contains(text(), 'Billing Period')]"))).valueContains(billingPeriod_BA2);
        });
        ...
        assertThat(tableModalPremium).hasMatchingRows(1, ImmutableMap.of(ModalPremiums.STATUS.getName(), "Active"));
        ...
        assertThat((tableModalPremium.getRow(ImmutableMap.of(ModalPremiums.LOCATION.getName(), "LOC2", ModalPremiums.MODAL_PREMIUM_EFFECTIVE_DATE.getName(), currentDate)).getCell(ModalPremiums.COVERAGE.getName()).controls.links).getFirst()).isAbsent();
