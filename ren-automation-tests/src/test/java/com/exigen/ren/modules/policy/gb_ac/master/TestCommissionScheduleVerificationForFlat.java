...
import static com.exigen.ren.main.modules.policy.gb_ac.masterpolicy.metadata.PolicyInformationTabMetaData.AssignedAgenciesMetaData.INDEPENDENT_COMMISSIONABLE_PRODUCER_ICP;
import static com.exigen.ren.utils.components.Components.POLICY_GROUPBENEFITS;
import static com.exigen.ren.utils.groups.Groups.*;

public class TestCommissionScheduleVerificationForFlat extends BaseTest implements CustomerContext, CaseProfileContext, GroupAccidentMasterPolicyContext {

    @Test(groups = {GB, GB_PRECONFIGURED, GB_AC, WITHOUT_TS, REGRESSION})
    @TestInfo(testCaseId = "REN-26766", component = POLICY_GROUPBENEFITS)
    public void testCommissionScheduleVerificationForFlat_AC() {
        ...
        tableCommissionRules.getRowsCount();

        if (tableCommissionRules.getRowsCount() > 8 ){
            tableCommissionRules.getColumn(GBCommissionRuleTab.CommissionRules.ACTIONS).getCell(9).controls.links.get(DELETE).click();
        ...
        groupAccidentMasterPolicy.getDefaultWorkspace().fillFromTo(getDefaultACMasterPolicyData(), EnrollmentTab.class, PremiumSummaryTab.class, true);
    ...
    private void commissionsVerifications(String agencyName, ETCSCoreSoftAssertions softly) {
        softly.assertThat(PremiumSummaryTab.getCommissionTypeByAgency(agencyName, ENHANCED_ACCIDENT)).isEqualTo("Flat");
        softly.assertThat(PremiumSummaryTab.getCommissionTable(agencyName, ENHANCED_ACCIDENT).getHeader().getValue()).containsExactly(TableConstants.AgencyCommission.COVERAGE_NAME.getName(), RATE.getName());

        Button overrideCommissionButton = PremiumSummaryTab.getCommissionOverrideButtonForAgencyWithCoverage(agencyName, ENHANCED_ACCIDENT);
