...
import static com.exigen.ren.main.modules.claim.common.metadata.PaymentPaymentPaymentDetailsTabMetaData.STATE;
...
public class TestSynchronizeInClaimAddressFields extends ClaimGroupBenefitsTLBaseTest {

    private PaymentPaymentPaymentDetailsTab paymentDetailsTab = claim.addPayment().getWorkspace().getTab(PaymentPaymentPaymentDetailsTab.class);
    ...
    public void testSynchronizeInClaimAddressFields() {
        ...
        TestData tdClaimCreation = termLifeClaim.getDefaultTestData(DATA_GATHER, DEFAULT_TEST_DATA_KEY);
        claim.initiate(tdClaimCreation);
        claim.getDefaultWorkspace().fillUpTo(tdClaimCreation, policyInformationSponsorTab.getClass(), false);
        ...
        claim.getDefaultWorkspace().fillFromTo(tdClaimCreation
                        ...
                        .adjust(makeKeyPath(additionalPartiesAdditionalPartyTab.getMetaKey(), PREFERRED_PAYMENT_METHOD_BENEFIT.getLabel()), "Check")
        ...
        additionalPartiesAdditionalPartyTab.navigate();
        assertSoftly(softly -> {
            ...
            softly.assertThat(policyInformationSponsorTab.navigateToTab().getAssetList().getAsset(SPONSOR_ADDRESS)).hasValue(String.format("%s %s %s %s %s %s %s", mapSecondAddress.get(ADDRESS_LINE_1),
                    mapSecondAddress.get(ADDRESS_LINE_2), mapSecondAddress.get(ADDRESS_LINE_3), mapSecondAddress.get(CITY), mapSecondAddress.get(STATE_PROVINCE), mapSecondAddress.get(ZIP_POST_CODE), mapSecondAddress.get(COUNTRY)));
        });
        ...
        claim.addPayment().getWorkspace().fillFromTo(tdClaim.getTestData("ClaimPayment", "TestData_FinalPayment"), PaymentPaymentPaymentAllocationTab.class, PaymentPaymentAdditionalPayeeTab.class);
