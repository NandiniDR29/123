...
public class TestPolicyYearUpdateJobVerification extends BaseTest implements CustomerContext, CaseProfileContext, PaidFamilyLeaveMasterPolicyContext {

    @Test(groups = {GB, GB_PRECONFIGURED, GB_PFL, WITHOUT_TS, REGRESSION})
    @TestInfo(testCaseId = "REN-29905", component = POLICY_GROUPBENEFITS)
    public void policyYearUpdateJobVerification() {
        ...
        masterPolicyCreation(currentDate, currentDate);
        performEndorsement(currentDate.plusMonths(1));// failed by REN-33079
        performEndorsement(currentDate);// failed by REN-33079
        ...
        paidFamilyLeaveMasterPolicy.policyInquiry().start();
    ...
    private void masterPolicyCreation(LocalDateTime effectiveDate, LocalDateTime nextPolicyYearStartDate) {
        paidFamilyLeaveMasterPolicy.createPolicyViaUI(getDefaultPFLMasterPolicyData()
                ...
                .adjust(TestData.makeKeyPath(policyInformationTab.getMetaKey(), PolicyInformationTabMetaData.NEXT_RENEWAL_QUOTE_START_DATE.getLabel()),
                        effectiveDate.format(DateTimeUtils.MM_DD_YYYY))
    ...
    private void performEndorsement(LocalDateTime endorsementDate) {
        paidFamilyLeaveMasterPolicy.createEndorsement(paidFamilyLeaveMasterPolicy.getDefaultTestData(TestDataKey.ENDORSEMENT, TestDataKey.DEFAULT_TEST_DATA_KEY)
                ...
                .adjust(paidFamilyLeaveMasterPolicy.getDefaultTestData(TestDataKey.DATA_GATHER, "TestDataUpdateSomething").resolveLinks())
